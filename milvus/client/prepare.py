...
from .types import DataType, MetricType, PlaceholderType
...
class Prepare:
                        ...
                        for ik, iv in index_param.items():
                            if ik == "metric_type":
                                if not isinstance(iv, MetricType) and not isinstance(iv, str):
                                    raise ParamError(
                                        "metric_type must be of Milvus.MetricType or str!")
    ...
    def create_partition_request(cls, collection_name, partition_name):
        return milvus_types.CreatePartitionRequest(collection_name=collection_name,
                                                   partition_name=partition_name)

    @classmethod
    def drop_partition_request(cls, collection_name, partition_name):
        return milvus_types.DropPartitionRequest(collection_name=collection_name,
                                                 partition_name=partition_name)

    @classmethod
    def has_partition_request(cls, collection_name, partition_name):
        return milvus_types.HasPartitionRequest(collection_name=collection_name,
                                                partition_name=partition_name)

    @classmethod
    def partition_stats_request(cls, collection_name, partition_name):
        return milvus_types.PartitionStatsRequest(collection_name=collection_name,
                                                  partition_name=partition_name)
    ...
    @classmethod
    def bulk_insert_param(cls, collection_name, entities, partition_tag, ids=None,
                          params=None, fields_info=None, **kwargs):
        default_partition_tag = "_default"  # should here?
        tag = partition_tag or default_partition_tag
        insert_request = milvus_types.InsertRequest(collection_name=collection_name,
                                                    partition_name=tag)

        for entity in entities:
            if not entity.get("name", None) or \
               not entity.get("values", None) or \
               not entity.get("type", None):
                raise ParamError(
                    "Missing param in entities, a field must have type, name and values")
                ...
                for j in range(fields_len):
                    ...
                    if field_name == entity_name:
                        if field_type != entity_type:
                            raise ParamError(
                                "The type of {} doesn't match. \
                                Field type of collection is {}, \
                                but the field type of entities is {}".format(field_name,
                                                                             field_type,
                                                                             entity_type))
                        ...
                        if field_type in [DataType.FLOAT_VECTOR, DataType.BINARY_VECTOR]:
                            ...
                            if entity_type == DataType.BINARY_VECTOR:
                                entity_dim *= 8

                            if field_dim != entity_dim:
                                raise ParamError(
                                    "The dimension of {0} doesn't match. \
                                    Dimension of {0} in collection is {1}, \
                                    but the dimension of entities is {2}".format(field_name,
                                                                                 field_dim,
                                                                                 entity_dim))

                        location[field["name"]] = j
                        fields_type.append(entity_type)
                        match_flag = True
                        break
    ...
    # pylint: disable=too-many-statements
    @classmethod
    def insert_param(cls, collection_name, entities, partition_tag, ids=None,
                     params=None, fields_info=None, **kwargs):
        ...
        if fields_info is None:
            ...
            for field_info in fields_info:
                ...
                fields_type.append(field_info["type"])
    ...
    @classmethod
    def _extract_vectors_param(cls, param, placeholders, meta=None):
        ...
        if isinstance(param, dict):
            ...
            for _, v in param.items():
                cls._extract_vectors_param(v, placeholders, meta)

        if isinstance(param, list):
            for item in param:
                cls._extract_vectors_param(item, placeholders, meta)
    ...
    @classmethod
    def divide_search_request(cls, collection_name, query_entities,
        ...
        cls._extract_vectors_param(duplicated_entities, vector_placeholders, meta)
    ...
    def create_index__request(cls, collection_name, field_name, params):
        index_params = milvus_types.CreateIndexRequest(collection_name=collection_name,
    ...
    def describe_index_request(cls, collection_name, field_name):
        return milvus_types.DescribeIndexRequest(collection_name=collection_name,
                                                 field_name=field_name)

    @classmethod
    def describe_index_progress_request(cls, collection_name, field_name):
        return milvus_types.DescribeIndexProgressRequest(collection_name=collection_name,
                                                         field_name=field_name)

    @classmethod
    def get_index_state_request(cls, collection_name, field_name):
        return milvus_types.GetIndexStateRequest(collection_name=collection_name,
                                                 field_name=field_name)
    ...
    def release_collection(cls, db_name, collection_name):
        return milvus_types.ReleaseCollectionRequest(db_name=db_name,
                                                     collection_name=collection_name)
    ...
    def release_partitions(cls, db_name, collection_name, partition_names):
        return milvus_types.ReleasePartitionsRequest(db_name=db_name,
                                                     collection_name=collection_name,
                                                     partition_names=partition_names)
    ...
    def drop_index_request(cls, collection_name, field_name, index_name):
        return milvus_types.DropIndexRequest(db_name="",
                                             collection_name=collection_name,
                                             field_name=field_name,
