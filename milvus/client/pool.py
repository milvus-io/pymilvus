...
def _is_version_match(version):
    ...
    for support_version in support_versions:
        ...
        if version_prefix[0] == support_version_prefix[0] and \
           version_prefix[1] == support_version_prefix[1]:
            return True
    return False
...
class ConnectionRecord:
    def __init__(self, uri, handler="GRPC", conn_id=-1, pre_ping=True, **kwargs):
        ...
        # if handler == "GRPC":
        #     self._register_link()
        #     self._connection = GrpcHandler(uri=self._uri, pre_ping=self._pre_ping,
        #                                    conn_id=conn_id, **self._kw)
        else:
            raise ValueError("Unknown handler type. Use GRPC")

    def _register_link(self):
        with RegistryHandler(uri=self._uri, pre_ping=self._pre_ping,
                             conn_id=self._conn_id, **self._kw) as register:
