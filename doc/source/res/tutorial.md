...
> If we create collection with `auto_id = True`, we can't provide ids of our own, and the returned
 `ids` is automatically generated by Milvus. If `partition_tag` isn't provided, these entities will be inserted
...
> If we don't provide anything in `"fields"`, there will only be ids and distances in the results. Only what we
