...
def init_data(connect, collection, nb=1200, partition_names=None, auto_id=True):
    '''
    ...
    if partition_names is None:
        ...
        if auto_id:
            ids = connect.bulk_insert(collection, insert_entities, partition_name=partition_names)
        else:
            ids = connect.bulk_insert(collection, insert_entities, ids=[i for i in range(nb)], partition_name=partition_names)
    # connect.flush([collection])
    return insert_entities, ids


def init_binary_data(connect, collection, nb=1200, insert=True, partition_names=None):
    '''
    Generate entities and add it in collection
    '''
    ids = []
...
class TestSearchBase:
    ...
    def test_search_index_partition_B(self, connect, collection, get_simple_index, get_top_k, get_nq):
        ...
        for tags in [[default_tag], [default_tag, "new_tag"]]:
            if top_k > max_top_k:
                with pytest.raises(Exception) as e:
                    res = connect.search(collection, query, partition_names=tags)
            else:
                res = connect.search(collection, query, partition_names=tags)
                assert len(res) == nq
                assert len(res[0]) >= top_k
...
class TestSearchWithExpr(object):
    def test_search_with_expression_naive(self, connect):
        collection = "test_search_with_expression_naive"
        year_field = "year"
        vec_field = "vec"
        dim = 128
        nb = 3000
        ...
        fields = {
            "fields": [
                {"name": year_field, "type": DataType.INT64},
                {"name": vec_field, "type": DataType.FLOAT_VECTOR,
                 "params": {"dim": dim},
                 "indexes": [{"metric_type": "L2"}]},
            ],
            "auto_id": True
        }
        if connect.has_collection(collection):
            connect.drop_collection(collection)
        connect.create_collection(collection, fields)

        vectors = [[random.random() for _ in range(dim)] for _ in range(nb)]
        vectors = preprocessing.normalize(vectors, axis=1, norm='l2').tolist()
        entities = [
            {"name": year_field, "type": DataType.INT64, "values": [i for i in range(nb)]},
            {"name": vec_field, "type": DataType.FLOAT_VECTOR, "values": vectors}
        ]
        connect.insert(collection, entities)

        connect.create_index(collection, vec_field, {"index_type": "IVF_FLAT", "metric_type": "L2", "params": {"nlist": 100}})
        ...
        connect.search(collection, dsl)
        connect.search_with_expression(collection, data, vec_field, param, topk, expr)
