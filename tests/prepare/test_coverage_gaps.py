...
class TestSnapshotRequests:
    """Additional tests for snapshot requests."""
    ...
    def test_create_snapshot_invalid_name(self):
        ...
        with pytest.raises(ParamError):
            Prepare.create_snapshot_req("", "test_coll")

    def test_create_snapshot_invalid_collection(self):
        """Test create snapshot with invalid collection name."""
        with pytest.raises(ParamError):
    ...
    def test_list_snapshots(self):
        ...
        req = Prepare.list_snapshots_req("test_coll")
        assert req.collection_name == "test_coll"
    ...
    def test_get_restore_snapshot_state(self):
        ...
        req = Prepare.get_restore_snapshot_state_req(12345)
        assert req.job_id == 12345

    def test_get_restore_snapshot_state_invalid_id(self):
        """Test get restore snapshot state with invalid ID."""
        with pytest.raises(ParamError):
            Prepare.get_restore_snapshot_state_req(0)
