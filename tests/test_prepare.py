...
class TestPrepare:
    ...
    def test_search_requests_with_expr_offset(self):
        ...
        search_params = {"metric_type": "L2", "offset": 10, "params": {"page_retain_order": True}}
...
class TestCreateCollectionRequest:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            },
            {
                "fields": [
                    {
                        "name": "test_floatvector",
                        "type": DataType.FLOAT_VECTOR,
                        "params": {
                            "dim": 128,
                            DefaultConfig.MaxVarCharLengthKey: DefaultConfig.MaxVarCharLength + 1,
                        },
                    },
                ]
            },
    ...
    def test_row_insert_param_with_auto_id(self):
        ...
        Prepare.row_insert_param(
            "", rows, "", fields_info=schema.to_dict()["fields"], enable_dynamic=True
        )
        ...
        rows = [
            {
                ...
                "nullable_field": None,
                "default_field": None,
                "float_vector": rng.random((1, dim))[0],
                "a": 1,
            },
            {"float": 1.0, "float_vector": rng.random((1, dim))[0], "b": 1},
        ]
    ...
    def test_row_upsert_param_with_partial_update_true(self):
        ...
        request = Prepare.row_upsert_param(
            "test_collection",
            rows,
            "",
        ...
        assert (
            request.partial_update is True
        ), "partial_update should be True when explicitly set to True"
    ...
    def test_row_upsert_param_partial_update_default(self):
        ...
        request = Prepare.row_upsert_param(
            "test_collection", rows, "", fields_info=schema.to_dict()["fields"], enable_dynamic=True
        )
    ...
    def test_batch_upsert_param_with_partial_update_true(self):
        ...
        request = Prepare.batch_upsert_param(
            "test_collection", entities, "", fields_info, partial_update=True
        )
    ...
    def test_batch_upsert_param_partial_fields_with_partial_update_true(self):
        ...
        entities = [
            {"name": "id", "type": DataType.INT64, "values": [1, 2, 3]},
            {
                "name": "name",
                "type": DataType.VARCHAR,
                "values": ["updated_a", "updated_b", "updated_c"],
            },
    ...
    def test_batch_upsert_param_partial_fields_with_partial_update_false_should_fail(self):
        ...
        with pytest.raises(ParamError, match="expected number of fields"):
            Prepare.batch_upsert_param(
                "test_collection", entities, "", fields_info, partial_update=False
            )
    ...
    def test_row_upsert_param_missing_fields_partial_update_false_should_fail(self):
        ...
        rows = [{"pk_field": 1, "float_vector": rng.random((1, dim))[0]}]  # Missing required_field
    ...
    def test_row_upsert_param_partial_update_with_float16_vector_cached(self):
        ...
        rows = [{"pk": 1, "float16_vector": vector_data[0]}]

        req = Prepare.row_upsert_param(
            "test_collection", rows, "", fields_info=schema.to_dict()["fields"], partial_update=True
        )

        vector_field_data = next(
            (fd for fd in req.fields_data if fd.field_name == "float16_vector"), None
        )
        ...
        assert (
            len(vector_field_data.vectors.float16_vector) == expected_bytes
        ), f"Vector data has incorrect size! Expected {expected_bytes} bytes."
...
class TestCreateIndexRequest:
    def test_create_index_request_with_false_boolean_param(self):
        ...
        assert (
            param_values["with_raw_data"] == "False"
        ), "with_raw_data should be serialized as 'False'"
        assert "index_type" in param_keys
        # String values are serialized as plain strings (without quotes) for compatibility
        assert (
            param_values["index_type"] == "SCANN"
        ), "index_type should be serialized as plain string"
        assert (
            param_values["metric_type"] == "L2"
        ), "metric_type should be serialized as plain string"
    ...
    def test_create_index_request_with_true_boolean_param(self):
        ...
        # Short-term fix: booleans are serialized as uppercase "False"/"True" for e2e compatibility
        assert (
            param_values["with_raw_data"] == "True"
        ), "with_raw_data should be serialized as 'True'"
        assert (
            param_values["index_type"] == "SCANN"
        ), "index_type should be serialized as plain string"
        assert (
...
class TestFunctionEditor:
    def test_add_function(self):
        req = Prepare.add_collection_function_request(
            "test_collection",
            Function(
                "test",
    ...
    def test_alter_function(self):
        req = Prepare.alter_collection_function_request(
            "test_collection",
            "test",
            Function(
                "test",
    ...
    def test_drop_function(self):
        req = Prepare.drop_collection_function_request("test_collection", "test")
        assert req.collection_name == "test_collection"
...
class TestSnapshotRequests:
    ...
    def test_list_snapshots_req(self):
        ...
        req = Prepare.list_snapshots_req(collection_name="test_collection")
        assert req.collection_name == "test_collection"
    ...
    def test_restore_snapshot_req(self):
        ...
        req = Prepare.restore_snapshot_req(
            snapshot_name="test_snapshot", collection_name="test_collection", rewrite_data=True
        )
        assert req.name == "test_snapshot"
        assert req.collection_name == "test_collection"
    ...
    def test_restore_snapshot_req_invalid_collection_name(self, invalid_name):
        ...
        with pytest.raises(MilvusException):
            Prepare.restore_snapshot_req(
                snapshot_name="test_snapshot", collection_name=invalid_name
            )
