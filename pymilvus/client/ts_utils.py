...
class GTsDict(metaclass=Singleton):
    ...
    def get(self, collection: int) -> int:
...
def update_ts_on_mutation(collection: int) -> int:
...
def get_collection_ts(collection: int) -> int:
    return _get_gts_dict().get(collection)


# Get the last write timestamp of collection.
def get_collection_timestamp(collection: int) -> int:
...
def get_collection_datetime(
    collection: int, tz: Optional[datetime.timezone] = None
) -> datetime.datetime:
    timestamp = get_collection_timestamp(collection)
    return datetime.datetime.fromtimestamp(timestamp, tz=tz)


def get_eventually_ts() -> int:
    return EVENTUALLY_TS


def get_bounded_ts() -> int:
    return BOUNDED_TS


def construct_guarantee_ts(collection_name: str, kwargs: Dict) -> Any:
