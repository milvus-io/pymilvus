...
class Prepare:
    ...
    def _parse_row_request(
        ...
        primary_field_info = next((field for field in fields_info if field.get("is_primary", False)), None)
        if primary_field_info and primary_field_info.get("auto_id", False):
            if entities and primary_field_info["name"] in entities[0]:
