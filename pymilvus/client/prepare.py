    ...
    def search_requests_with_expr(
        ...
        **kwargs,
        ...
        if isinstance(ranker, FunctionScore):
            request.function_score = Prepare.function_score_schema(ranker)
    ...
    def function_score_schema(function_score: FunctionScore) -> schema_types.FunctionScore:
        functions = [
                schema_types.FunctionSchema(
                    ...
                    params= [common_types.KeyValuePair(key=str(k), value=Prepare.common_kv_value(v)) for k,v in ranker.params.items()]
                )
        ...
        return schema_types.FunctionScore(
            functions=functions,
            params=[common_types.KeyValuePair(key=str(k), value=Prepare.common_kv_value(v)) for k,v in function_score.params.items()]
        )
