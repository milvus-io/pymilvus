...
from ..exceptions import DataNotMatchException, ExceptionsMessage, ParamError
...
from . import blob, entity_helper
...
class Prepare:
    ...
    def create_collection_request(
        ...
        **kwargs,
        ...
        if isinstance(fields, CollectionSchema):
            schema = cls.get_schema_from_collection_schema(
                collection_name, fields, shards_num, **kwargs
            )
        else:
    ...
    def get_schema_from_collection_schema(
        cls, collection_name: str, fields: CollectionSchema, shards_num=2, **kwargs
    ) -> milvus_types.CreateCollectionRequest:
        ...
        schema = schema_types.CollectionSchema(
            name=collection_name, autoID=fields.auto_id, description=coll_description
        )
    ...
    @classmethod
    def get_schema(
        cls, collection_name: str, fields: Dict[str, Iterable], shards_num=2, **kwargs
        ...
        for field in all_fields:
            ...
            if is_primary:
                if primary_field is not None:
                    raise ParamError(
                        message="A collection should only have one primary field"
                    )
                if DataType(data_type) not in [DataType.INT64, DataType.VARCHAR]:
                    raise ParamError(
                        message="int64 and varChar are the only supported types of primary key"
                    )
            ...
            if auto_id:
                if auto_id_field is not None:
                    raise ParamError(
                        message="A collection should only have one autoID field"
                    )
                if DataType(data_type) != DataType.INT64:
                    raise ParamError(
                        message="int64 is the only supported type of automatic generated id"
                    )
    ...
    def alter_collection_request(cls, collection_name, properties):
        ...
        return milvus_types.AlterCollectionRequest(
            collection_name=collection_name, properties=kvs
        )
    ...
    def show_collections_request(cls, collection_names=None):
        ...
        if collection_names:
            if not isinstance(collection_names, (list,)):
                raise ParamError(
                    message=f"collection_names must be a list of strings, but got: {collection_names}"
                )
    ...
    def show_partitions_request(
        cls, collection_name, partition_names=None, type_in_memory=False
    ):
        check_pass_param(
            collection_name=collection_name, partition_name_array=partition_names
        )
        req = milvus_types.ShowPartitionsRequest(collection_name=collection_name)
        if partition_names:
            if not isinstance(partition_names, (list,)):
                raise ParamError(
                    message=f"partition_names must be a list of strings, but got: {partition_names}"
                )
    ...
    def partition_name(cls, collection_name, partition_name):
        ...
        return milvus_types.PartitionName(
            collection_name=collection_name, tag=partition_name
        )

    @classmethod
    def batch_insert_param(
        cls, collection_name, entities, partition_name, fields_info=None, **kwargs
    ):
        ...
        if auto_id_loc is None and len(entities) != len(fields_info):
            raise ParamError(
                message=f"number of fields: {len(fields_info)}, number of entities: {len(entities)}"
            )
        ...
        try:
            ...
            raise DataNotMatchException(
                message=ExceptionsMessage.DataTypeInconsistent
            ) from e
    ...
    def delete_request(cls, collection_name, partition_name, expr):
        ...
        request = milvus_types.DeleteRequest(
            collection_name=collection_name, expr=expr, partition_name=partition_name
        )
        return request
    ...
    def search_request(
        ...
        **kwargs,
        ...
        fields_name_locs = {
            fields_schema[loc]["name"]: loc for loc in range(len(fields_schema))
        }

        if not isinstance(query_entities, (dict,)):
            raise ParamError(
                message="Invalid query format. 'query_entities' must be a dict"
            )
            ...
            if isinstance(param, dict):
                if "vector" in param:
                    ...
                    for pk, pv in param["vector"].items():
                        if "query" not in pv:
                            raise ParamError(
                                message="param vector must contain 'query'"
                            )
        ...
        extract_vectors_param(
            duplicated_entities, vector_placeholders, vector_names, round_decimal
        )
        ...
        for tag, vectors in vector_placeholders.items():
            ...
            dimension = int(
                fields_schema[fields_name_locs[fname]]["params"].get("dim", 0)
            )
            ...
            if isinstance(vectors[0], bytes):
                ...
                for vector in vectors:
                    if dimension != len(vector) * 8:
                        raise ParamError(
                            message="The dimension of query vector is different from schema"
                        )
    ...
    def search_requests_with_expr(
        ...
        **kwargs,
        ...
        dimension = int(
            fields_schema[fields_name_locs[anns_field]]["params"].get("dim", 0)
        )

        params = param.get("params", {})
        if not isinstance(params, dict):
            raise ParamError(
                message=f"Search params must be a dict, got {type(params)}"
            )
        search_params = {
    ...
    def create_alias_request(cls, collection_name, alias):
        return milvus_types.CreateAliasRequest(
            collection_name=collection_name, alias=alias
        )
    ...
    def alter_alias_request(cls, collection_name, alias):
        return milvus_types.AlterAliasRequest(
            collection_name=collection_name, alias=alias
        )
    ...
    def get_collection_stats_request(cls, collection_name):
        return milvus_types.GetCollectionStatisticsRequest(
            collection_name=collection_name
        )
    ...
    @classmethod
    def query_request(
        cls, collection_name, expr, output_fields, partition_names, **kwargs
    ):
        ...
        if limit is not None:
            req.query_params.append(
                common_types.KeyValuePair(key="limit", value=str(limit))
            )

        offset = kwargs.get("offset", None)
        if offset is not None:
            req.query_params.append(
                common_types.KeyValuePair(key="offset", value=str(offset))
            )
    ...
    def load_balance_request(
        cls, collection_name, src_node_id, dst_node_ids, sealed_segment_ids
    ):
    ...
    def create_role_request(cls, role_name):
        ...
        return milvus_types.CreateRoleRequest(
            entity=milvus_types.RoleEntity(name=role_name)
        )
    ...
    def operate_privilege_request(
        cls, role_name, object, object_name, privilege, operate_privilege_type
    ):
        check_pass_param(role_name=role_name)
