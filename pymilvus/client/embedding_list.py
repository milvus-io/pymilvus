...
class EmbeddingList:
    ...
    Examples:
        ...
        >>> embeddings = model.encode(data)  # Get embeddings from your model
        >>> query = EmbeddingList(embeddings)
        >>> 
    ...
    def __init__(self, embeddings: Optional[Union[List[np.ndarray], np.ndarray]] = None, 
        ...
        if embeddings is not None:
            if isinstance(embeddings, np.ndarray):
                if embeddings.ndim == 1:
                    ...
                    raise ValueError(f"Embeddings array must be 1D or 2D, got {embeddings.ndim}D")
            elif isinstance(embeddings, list):
                for emb in embeddings:
                    self.add(emb)
            else:
                raise TypeError(f"Embeddings must be numpy array or list, got {type(embeddings)}")
    ...
    def _parse_dtype(self, dtype: Union[np.dtype, str, DataType]) -> np.dtype:
        ...
        if isinstance(dtype, np.dtype):
            ...
            raise TypeError(f"dtype must be numpy dtype, string, or DataType, got {type(dtype)}")
    ...
    def add(self, embedding: Union[np.ndarray, List[Any]]) -> 'EmbeddingList':
        ...
        if self._embeddings:
            if len(embedding) != self.dim:
                raise ValueError(f"Embedding dimension {len(embedding)} doesn't match existing {self.dim}")
        elif self._dim is not None:
            if len(embedding) != self._dim:
                raise ValueError(f"Embedding dimension {len(embedding)} doesn't match expected {self._dim}")
    ...
    def to_flat_array(self) -> np.ndarray:
        ...
        if not self._embeddings:
            raise ValueError("EmbeddingList is empty")
