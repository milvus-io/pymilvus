...
from ..client.check import is_legal_address, is_legal_host, is_legal_port
...
from .default_config import ENV_CONNECTION_CONF, DefaultConfig
...
class Connections(metaclass=SingleInstanceMetaClass):
    ...
    def _read_default_config_from_os_env(self):
        ...
        if not matched:
            raise ConnectionConfigException(
                message=ExceptionsMessage.EnvConfigErr % (ENV_CONNECTION_CONF, conf)
            )
        ...
        return {"user": user, "address": f"{host}:{port}"}
        ...
        for alias, config in kwargs.items():
            ...
            if alias in self._connected_alias:
                if self._alias[alias].get("address") != addr:
                    raise ConnectionConfigException(
                        message=ExceptionsMessage.ConnDiffConf % alias
                    )
    ...
    def __generate_address(self, uri: str, host: str, port: str) -> str:
        ...
        if uri != "":
            ...
            addr = (
                ...
                else f"{parsed_uri.netloc}:{DefaultConfig.DEFAULT_PORT}"
            )
        ...
        if not 0 <= int(port) < 65535:
            raise ConnectionConfigException(
                message=f"port number {port} out of range, valid range [0, 65535)"
            )
    ...
    def disconnect(self, alias: str):
        ...
        if not isinstance(alias, str):
            raise ConnectionConfigException(
                message=ExceptionsMessage.AliasType % type(alias)
            )
    ...
    def connect(
        self, alias=DefaultConfig.DEFAULT_USING, user="", password="", **kwargs
    ):
        """
        ...
        if with_config(config):
            ...
            if alias not in self._alias:
                raise ConnectionConfigException(
                    message=ExceptionsMessage.ConnLackConf % alias
                )
