...
def retry_on_rpc_failure(
    ...
    def wrapper(func):
        ...
        def handler(self, *args, **kwargs):
            ...
            retry_timeout = (
                _timeout if _timeout is not None and isinstance(_timeout, int) else None
            )
            counter = 1
            ...
            while True:
                try:
                    ...
                    if timeout(start_time):
                        ...
                        if e.code() == grpc.StatusCode.DEADLINE_EXCEEDED:
                            raise MilvusException(
                                message=f"rpc deadline exceeded: {timeout_msg}"
                            ) from e
                        if e.code() == grpc.StatusCode.UNAVAILABLE:
                            raise MilvusUnavailableException(
                                message=f"server Unavailable: {timeout_msg}"
                            ) from e
                        raise MilvusException(message=str(e)) from e
                    ...
                    if timeout(start_time):
                        ...
                        raise MilvusException(
                            e.code, f"{timeout_msg}, message={e.message}"
                        ) from e
    ...
    def wrapper(func):
        ...
        def handler(*args, **kwargs):
            ...
            try:
                ...
                LOGGER.error(
                    f"grpc Timeout: [{inner_name}], <{e.__class__.__name__}: {e.code()}, {e.details()}>, <Time:{record_dict}>"
                )
                raise e
            except grpc.RpcError as e:
                record_dict["gRPC error"] = str(datetime.datetime.now())
                LOGGER.error(
                    f"grpc RpcError: [{inner_name}], <{e.__class__.__name__}: {e.code()}, {e.details()}>, <Time:{record_dict}>"
                )
                raise e
            except Exception as e:
                record_dict["Exception"] = str(datetime.datetime.now())
                LOGGER.error(
                    f"Unexcepted error: [{inner_name}], {e}, <Time: {record_dict}>"
                )
                raise e
